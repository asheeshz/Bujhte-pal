...
(function() { // IIFE for VSW
    ...
    */
    ...
    function vsw_checkInputsAndToggleSearchButton(searchBoxElement){
         ...
         const minInputsRequired=parseInt(searchBoxElement.dataset.minInputs) || 2; // एचटीएमएल से minInputs प्राप्त करें या 2 डिफ़ॉल्ट करें
    ...
    async function vsw_fetchYouTubeData(searchTerm=''){
        // API की HTML एलिमेंट से प्राप्त करें
        const apiKeyElement = document.getElementById('vsw-api-key');
        const apiKey = apiKeyElement ? apiKeyElement.textContent.trim() : '';

        // API की की बुनियादी जाँच (HTML से प्राप्त की गई की के लिए)
        if(!apiKey || apiKey === 'YOUR_API_KEY_HERE' || apiKey.length < 30){ // 'YOUR_API_KEY_HERE' प्लेसहोल्डर के लिए भी जाँच
             console.error("VSW Error: API Key config missing/invalid from HTML element #vsw-api-key. Please ensure a valid API key is configured.");
             vsw_showMessage(vsw_getTextById('vsw-msgApiKeyError') + (apiKey.startsWith('AIzaSyB') ? " (डेमो की)" : " (HTML में #vsw-api-key से कॉन्फ़िगर करें)"), 8000);
             vsw_hideVideoSections();vsw_clearVideoResults();return;
        }

        // यदि यह सार्वजनिक डेमो की है तो चेतावनी दें (AIzaSyB से शुरू होने वाली)
    ...
    function vsw_performSearchInternal(searchBoxId){
        ...
        const minInputsRequired=parseInt(searchBox.dataset.minInputs) || 2; // एचटीएमएल से minInputs प्राप्त करें या 2 डिफ़ॉल्ट करें
